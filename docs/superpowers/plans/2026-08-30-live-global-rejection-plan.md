...
- [ ] **Step 3: Implement** — the orchestration below.
  ...
  2. **Skip tracking by `subIndex`, never a scalar.** Keep `var skippedIds = Set<Int>()` for the WHOLE pass (a scalar `skipped += 1` could double-count the SAME sub across the dimension probe, sample build, and output stream). Helper `loadWarp(_ reg) -> (image: AstroImage, mask: [Float])?`: first `if isCancelled() || DispatchTime.now() >= deadline { throw Cancel }`; else load via the loader (digest-verified), `Warp.apply(rgb, transform: reg.transform.liftedToFullResolution())`, then `if reg.leveling != nil { GradientLeveler.apply(warped, subModel: leveling.sub, refModel: leveling.ref, scale: reg.effectiveScale) }`; on a load throw → `skippedIds.insert(reg.subIndex)` + return nil. Cache successes in `var loaded = [Int: (AstroImage, [Float])]()` keyed by `subIndex`.
  ...
  4. `let idxs = SubRegistration.sampleIndices(count: inGen.count, maxSampleFrames: maxSampleFrames)`. The dimension-probe frame enters the SAMPLE **only if its index ∈ `idxs`** — it does not auto-enter. Build the sample: for each `i` in `idxs`, reuse `loaded[inGen[i].subIndex]` if present else `loadWarp(inGen[i])`; drop failures. `center = robustCenter(sample.map { ($0.0, $0.1) })`.
  5. **Output — reuse under budget, stream when capped (P3, matches spec):** if `inGen.count ≤ maxSampleFrames` (**under budget**: `idxs == all`, so `loaded` already holds every survivor) → `clippedWeightedMean` over the cached `loaded` frames, **no disk re-read**. Else (**capped**) → `clippedWeightedMean(frames: { AnyIterator over inGen, `loadWarp`-ing each fresh, skipping any in `skippedIds`, same cancel/deadline guard }, center, scale, kappa)`.
