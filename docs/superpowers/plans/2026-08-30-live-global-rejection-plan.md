...
3. **freeze** the survivor set + compute `currentFreshnessKey()` once — from here it does not change. (The freeze genuinely holds because reject toggles are blocked once `importer.isGeneratingReplay` is set — Task 11 P2-1.)
...
- **P2-1 — finalization freeze: block reject once `end()` begins.** `toggleReject` today guards only `!isRestacking` (`AppModel.swift:956`); a reject can therefore land DURING finalization, after Task 10 froze the survivor set — so the written clean master and the UI reject state diverge. Add `&& !importer.isGeneratingReplay` to the `toggleReject` guard (`endSession` sets `importer.isGeneratingReplay = true` at its top, `AppModel.swift:1236`), and disable the StatsView reject toggle the same way: `.disabled(model.isRestacking || model.isGeneratingReplay)` (`StatsView.swift:82`). Test: a `toggleReject` after `isGeneratingReplay` is set is a no-op.
