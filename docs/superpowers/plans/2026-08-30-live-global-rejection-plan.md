...
3. **freeze** (under `regLock`) the survivor set, `currentFreshnessKey()`, AND `capturedActive = liveRejectionActive` — from here none change. (The freeze genuinely holds because reject toggles are blocked once `importer.isGeneratingReplay` is set — Task 11 P2-1.)
