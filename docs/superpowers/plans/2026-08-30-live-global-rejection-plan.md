...
- `SessionPipeline.configureLiveRejection(enabled:kappa:maxSampleBytes:)` (Task 11) — the **config-change invalidation path**: `FreshnessKey` encodes κ, so a κ (or budget/enabled) change must invalidate too. Under `regLock`: update `liveRejectionActive`/κ/budget, **recompute `_freshnessKey`**, and **clear a now-stale `publishedMaster`**. After releasing the lock, if `liveRejectionActive` and the change affects the clean master (κ or budget), `refiner.noteChanged()`.
...
3. **freeze** — read `frozenGen = engine.currentStackGeneration` FIRST (engine lock, released), THEN under `regLock` snapshot `frozenSurvivors = currentSurvivorsLocked(frozenGen)`, `frozenKey = _freshnessKey` (the cached field read **DIRECTLY** — NOT the public `currentFreshnessKey()`, which re-acquires `regLock` → deadlock on the non-recursive lock, same as Task 8), and `capturedActive = liveRejectionActive` (direct field read). From here none change (reject toggles are blocked once `importer.isGeneratingReplay` is set — Task 11 P2-1).
