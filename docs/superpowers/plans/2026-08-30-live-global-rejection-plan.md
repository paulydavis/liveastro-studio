...
extension GlobalCombineTests {
    ...
    func testClippedWeightedMeanKeepsSubFloorSpread() {
        // Near-flat set: values within sub-floor spread. MAD ≈ 0 → sigma = scaleFloor; each |v-center|
        // (≤ 1e-8) < kappa·scaleFloor (3e-7) → ALL kept (the floor must not over-reject real quantization
        // noise). Pins the scaleFloor rationale.
        let frames = [wimg(1,1,0.5, weight: 1), wimg(1,1,0.5 + 1e-8, weight: 1), wimg(1,1,0.5 - 1e-8, weight: 1)]
