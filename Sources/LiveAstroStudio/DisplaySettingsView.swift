...
struct DisplaySettingsView: View {
    ...
    @ViewBuilder private func histogramInset(_ counts: [Int]) -> some View {
        if !counts.isEmpty {
            ...
            // North-up rotation used to pad the corners with opaque black, which is
            // indistinguishable by value from genuinely crushed shadows — it reported "clipped
            // 5.2%" on an image whose adjustments clipped nothing, so this readout was suppressed
            // whenever the frame was rotated. NorthUpRotation now crops the padding away on every
